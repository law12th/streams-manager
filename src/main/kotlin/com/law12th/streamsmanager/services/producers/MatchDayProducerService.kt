...
class MatchDayProducerService(
    ...
    fun processMatchDayPayloads() {
        val payloads = payloadRepository.findByPayloadTypeAndProcessed(Constants.MD, false)
        ...
        if (payloads.any()) {
            payloads.forEach{ payload ->
                try {
                    ...
                    logger.error("Error occurred match day: ${e.message}")
                }
    ...
    companion object {
        private val logger = LoggerFactory.getLogger(MatchDayProducerService::class.java)
    }
}