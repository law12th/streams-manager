...
class PreMatchStatsProducerService(
    @Autowired
    ...
    fun processPreMatchStatsPayloads() {
       val payloads = payloadRepository.findByPayloadTypeAndProcessed(Constants.PRMS, false)
       ...
       if (payloads.any()) {
           payloads.forEach{ payload ->
               try {
                   ...
                   logger.error("Error occurred pre-match stats: ${e.message}")
               }
    ...
    companion object {
        private val logger = LoggerFactory.getLogger(PreMatchStatsProducerService::class.java)
    }
}