...
class PostMatchStatsProducerService(
    @Autowired
    ...
    fun processPostMatchStatsPayloads() {
        val payloads = payloadRepository.findByPayloadTypeAndProcessed(Constants.POMS, false)
        ...
        if (payloads.any()) {
            payloads.forEach{ payload ->
                try {
                    ...
                    logger.error("Error occurred post match stats: ${e.message}")
                }
    ...
    companion object {
        private val logger = LoggerFactory.getLogger(PostMatchStatsProducerService::class.java)
    }
}