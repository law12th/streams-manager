...
class PreMatchBettingOddsProducerService(
    @Autowired
    ...
    @Scheduled(cron = Constants.EveryAfterOneDayCronExpression, zone = Constants.Timezone)
    fun processPreMatchBettingOddsPayloads() {
        val payloads = payloadRepository.findByPayloadTypeAndProcessed(Constants.PBO, false)
        ...
        if (payloads.any()) {
            payloads.forEach{ payload ->
                try {
                    ...
                    logger.error("Error occurred pre match betting odds: ${e.message}")
                }
    ...
    companion object {
        private val logger = LoggerFactory.getLogger(PreMatchBettingOddsProducerService::class.java)
    }
}